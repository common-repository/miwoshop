...
class ControllerOpenbayEtsyProduct extends Controller {
	...
	public function links() {
		...
		$pagination->url = $this->url->link('openbay/etsy/itemLinks', 'token=' . $this->session->data['token'] . '&pg={page}', 'SSL');
	...
	public function listings() {
		...
		if (!isset($this->request->get['pg'])) {
			...
			$filter['page'] = $this->request->get['pg'];
		}
		...
		if (isset($listing_response['data']['error'])) {
			...
			$pagination->url = $this->url->link('openbay/etsy_product/listings', 'token=' . $this->session->data['token'] . '&pg={page}&' . http_build_query($filter), 'SSL');
