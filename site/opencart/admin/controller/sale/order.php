...
class ControllerSaleOrder extends Controller {
	...
	public function add() {
		...
		if ($this->validate()) {
			...
			if ($api_info) {
				...
				curl_setopt($curl, CURLOPT_URL, HTTPS_CATALOG . 'wp-admin/admin-ajax.php?action=miwoshop&page=miwoshop&option=com_miwoshop&format=raw&route=api/login');
	...
	public function delete() {
		...
		if (isset($this->session->data['cookie'])) {
			...
			curl_setopt($curl, CURLOPT_URL, HTTPS_CATALOG . 'wp-admin/admin-ajax.php?action=miwoshop&page=miwoshop&option=com_miwoshop&format=raw&route=api/order/delete&order_id=' . $this->request->get['order_id']);
			curl_setopt($curl, CURLOPT_COOKIE, $this->session->data['cookie']['name'].'=' . $this->session->data['cookie']['id'] . ';'); #miwoshop-start
	...
	protected function getList() {
		...
		$pagination->url = $this->url->link('sale/order', 'token=' . $this->session->data['token'] . $url . '&pg={page}', 'SSL');
	...
	public function history() {
		...
		$pagination->page = $page;
		$pagination->limit = 10;
		$pagination->url = $this->url->link('sale/order/history', 'token=' . $this->session->data['token'] . '&format=raw&tmpl=component&order_id=' . $this->request->get['order_id'] . '&pg={page}', 'SSL');
	...
	public function api() {
		...
		if (isset($this->session->data['cookie']) && isset($this->request->get['api'])) {
			...
			foreach ($this->request->get as $key => $value) {
				if ($key != 'route' && $key != 'token' && $key != 'client') {
					$url_data[$key] = $value;
				}
			}
			...
			curl_setopt($curl, CURLOPT_FORBID_REUSE, false);
			curl_setopt($curl, CURLOPT_RETURNTRANSFER, true);
			curl_setopt($curl, CURLOPT_URL, $url . 'wp-admin/admin-ajax.php?route=' . $this->request->get['api'] . ($url_data ? '&' . http_build_query($url_data) : ''));
			curl_setopt($curl, CURLOPT_POST, true);
