...
class ControllerCommonFileManager extends Controller {
	public function index() {
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => $this->url->link('common/filemanager', 'token=' . $this->session->data['token'] . '&format=raw&tmpl=component&action=miwoshop&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'))) . $url, 'SSL')
				);
		...
		$pagination->limit = 16;
		$pagination->url = $this->url->link('common/filemanager', 'token=' . $this->session->data['token'] . $url . '&pg={page}', 'SSL');
