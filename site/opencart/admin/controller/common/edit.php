...
class ControllerCommonEdit extends Controller {
    ...
    public function fixLanguage(){
        ...
        foreach($languages as $oc_lang){
            ...
            $j_lang_site_path  = MPATH_SITE . '/language/'. $oc_lang_code;
            $j_lang_admin_path = MPATH_SITE . '/administrator/language/'. $oc_lang_code;
            $oc_lang_site_path  = MPATH_MIWOSHOP_OC . '/catalog/language/'. $oc_lang['directory'];
            $oc_lang_admin_path = MPATH_MIWOSHOP_OC . '/admin/language/'. $oc_lang['directory'];
    ...
    public function copyLang(){
        $from	= MRequest::getInt('from', 0);
        $to 	= MRequest::getInt('to', 0);
    ...
    private function _fix($oc_lang_path, $j_lang_path, $oc_lang_code){
        $oc_files = MFolder::files($oc_lang_path, '.php', true, true);
        $vq_files = $this->_getVqFiles($oc_lang_path);

        $j_file = $j_lang_path.'/'.$oc_lang_code.'.com_miwoshop.ini';
        ...
        if(MFile::exists($j_file)) {
            MFile::delete($j_file);
            MFile::write($j_file, $ini_str);
        }
    }
    ...
    private function _getJoomlaKey($path, $key){
        ...
        if( count($path_array) > 0 ){
            $j_key ='COM_MIWOSHOP_'. strtoupper(implode('_', $path_array)) .'_'. strtoupper($key);
        }
        else{
            $j_key ='COM_MIWOSHOP_'. strtoupper($key);
        }
    ...
    private function _checkLangDirectories($od, $jd){
        $check = true;
        if(MFolder::exists($jd) == false){
            $check =false;
        }

        if(MFolder::exists($od) == false){
            $check =false;
        }
