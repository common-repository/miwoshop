...
class ControllerExtensionModification extends Controller {
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validate()) {
			foreach ($this->request->post['selected'] as $modification_id) {
                ...
                } else if(file_exists(MPATH_MIWOSHOP_OC . '/vqmod/xml/' . $modification_id)){
                    unlink(MPATH_MIWOSHOP_OC . '/vqmod/xml/' . $modification_id);
                }
                #miwoshop
			}
	...
	public function enable() {
		...
		if (isset($this->request->get['modification_id']) && $this->validate()) {
			...
			} else if(file_exists(MPATH_MIWOSHOP_OC . '/vqmod/xml/' . $this->request->get['modification_id'])){
                rename(MPATH_MIWOSHOP_OC . '/vqmod/xml/' . $this->request->get['modification_id'], MPATH_MIWOSHOP_OC . '/vqmod/xml/' . str_replace('.xml_', '.xml', $this->request->get['modification_id']));
            }
            #miwoshop
	...
	public function disable() {
		...
		if (isset($this->request->get['modification_id']) && $this->validate()) {
            ...
            if(file_exists(DIR_SYSTEM . 'xmls/' . $this->request->get['modification_id'])) {
                ...
                rename(MPATH_MIWOSHOP_OC . '/vqmod/xml/' . $this->request->get['modification_id'], MPATH_MIWOSHOP_OC . '/vqmod/xml/' . str_replace('.xml', '.xml_', $this->request->get['modification_id']));
            }
            #miwoshop
	...
	protected function getList() {
        ...
        $unvisible_xmls = array(
            'miwoshop_admin.xml',
            'miwoshop_buttons.xml',
            'miwoshop_catalog.xml',
            'miwoshop_filters.xml',
            'miwoshop_j_integration.xml',
            'miwoshop_system.xml',
            'miwoshop_triggers.xml',
            'x_miwoshop_finish.xml',
            'xx_miwoshop_wordpress',
            'miwoshop_module_random_product.xml',
            'miwoshop_payment_paypal_standart_billing_detail.xml'
        );
        ...
        foreach($files as $file) {
            ...
            // Do not show core MiwoShop xml files to users
		...
		$pagination->url = $this->url->link('extension/modification', 'token=' . $this->session->data['token'] . $url . '&pg={page}', 'SSL');
	...
	protected function _checkVqmodfile() {
        $files_enable  = glob(MPATH_MIWOSHOP_OC . '/vqmod/xml/*.xml');
        $files_disable = glob(MPATH_MIWOSHOP_OC . '/vqmod/xml/*.xml_');
