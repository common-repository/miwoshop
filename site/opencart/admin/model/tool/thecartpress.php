...
class ModelToolThecartpress extends Model {
  	...
  	public function importCategories($post){
        ...
        $cat = "SELECT t.term_id AS category_id, t.name AS category_name, tt.parent, tt.description AS category_desc FROM #__terms AS t INNER JOIN #__term_taxonomy AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = 'tcp_product_category' ORDER BY t.term_id";
		$db->setQuery($cat);
		...
		foreach($cats as $cat) {
			...
			/*$q = "SELECT meta_value FROM `#__jigoshop_termmeta` WHERE meta_key = 'thumbnail_id' AND jigoshop_term_id = ".$cat['category_id'];
			$db->setQuery($q);
  	...
  	public function _addCategoryPath() {
        $db = MFactory::getDBO();
		$db->setQuery("SELECT t.term_id AS id FROM `#__terms` AS t INNER JOIN `#__term_taxonomy` AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = 'tcp_product_category' ORDER BY t.term_id");
    ...
    private function _getPath($cat_id, $path = array()){
        $db = MFactory::getDBO();
        $db->setQuery("SELECT tt.parent FROM `#__terms` AS t INNER JOIN `#__term_taxonomy` AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = 'tcp_product_category' AND t.term_id = ".$cat_id);
  	...
  	public function importProducts($post) {
		...
		$q = "SELECT ID AS product_id, post_title, post_content, post_name AS product_sku, post_date, post_modified, post_status AS published, menu_order FROM #__posts WHERE post_type = 'tcp_product' AND post_parent = '0' AND post_status != 'trash' AND post_status != 'auto-draft' ORDER BY ID";
		$db->setQuery($q);
		...
		foreach($pros as $pro){
			$pro_image = $pro_price = $weight = $stock_quantity = $pro_sku = '';
            $stock_status = '7'; $pro_publish = '1';
			...
			$q = "INSERT IGNORE INTO `#__miwoshop_product` (`product_id`, `model`, `sku`, `location`, `quantity`, `stock_status_id`, `image`, `shipping`, `price`, `tax_class_id`, `status`,".
				"  `weight`, `weight_class_id`, `date_added`, `date_modified`, `date_available`, `sort_order`)".
				" VALUES ('".$pro['product_id']."', '".$pro_sku."', '".$pro_sku."', '', '".$stock_quantity."', '".$stock_status."', '".$pro_image."', '1', '".$pro_price."', '', '".$pro_publish.
				"', '".$weight."', '1', '".$datec."', '".$datem."',  '".$datec."', '".$pro['menu_order']."')";
			$db->setQuery($q);
			$db->query();
		...
		$q = "SELECT r.object_id, r.term_taxonomy_id FROM `#__posts` AS p INNER JOIN `#__term_relationships` AS r ON p.ID = r.object_id WHERE p.post_status != 'trash' AND p.post_type = 'tcp_product'";
		$db->setQuery($q);
