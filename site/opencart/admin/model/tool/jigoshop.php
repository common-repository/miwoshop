...
class ModelToolJigoshop extends Model {
    ...
    private function _getPath($cat_id, $path = array()){
  	...
  	public function importProducts($post) {
		...
		foreach($pros as $pro){
			$pro_image = $pro_sale_price = $pro_reg_price = $weight = $length = $width = $height = '';
			...
			$q = "INSERT IGNORE INTO `#__miwoshop_product` (`product_id`, `model`, `sku`, `location`, `quantity`, `stock_status_id`, `image`, `shipping`, `price`, `tax_class_id`, `status`,".
				...
				" VALUES ('".$pro['product_id']."', '".$pro['product_sku']."', '".$pro['product_sku']."', '', '".$stock_quantity."', '".$prod_stock."', '".$pro_image."', '1', '".$pro_reg_price."', '', '".$pro_publish.
