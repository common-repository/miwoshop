...
class ModelToolMarketpress extends Model {
  	...
  	public function importCategories($post){
        ...
        $cat = "SELECT t.term_id AS category_id, t.name AS category_name, tt.parent, tt.description AS category_desc FROM #__terms AS t INNER JOIN #__term_taxonomy AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = 'product_category' ORDER BY t.term_id";
		$db->setQuery($cat);
  	...
  	public function _addCategoryPath() {
        $db = MFactory::getDBO();
		$db->setQuery("SELECT t.term_id AS id FROM `#__terms` AS t INNER JOIN `#__term_taxonomy` AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = 'product_category' ORDER BY t.term_id");
    ...
    private function _getPath($cat_id, $path = array()){
        $db = MFactory::getDBO();
        $db->setQuery("SELECT tt.parent FROM `#__terms` AS t INNER JOIN `#__term_taxonomy` AS tt ON t.term_id = tt.term_id WHERE tt.taxonomy = 'product_category' AND t.term_id = ".$cat_id);
  	...
  	public function importProducts($post) {
		...
		$q = "SELECT ID AS product_id, post_title, post_content, post_name AS product_sku, post_date, post_modified, post_status AS published, menu_order FROM #__posts WHERE post_type = 'product' AND post_parent = '0' AND post_status != 'trash' AND post_status != 'auto-draft' ORDER BY ID";
		$db->setQuery($q);
		...
		foreach($pros as $pro){
			$pro_image = $pro_sale_price = ''; $pro_publish = '1'; $pro_price = '0.00';
			...
			foreach($metas as $meta){
				switch($meta['meta_key']){
					case 'mp_price':
                        ...
                        if(!empty($pro_reg_price)){
                            $pro_reg_price = array($pro_reg_price[0]);
                            foreach ($pro_reg_price as $key => $price) {
                                $pro_price = $pro_reg_price[$key];
                            }
                        }
						break;
					case '_thumbnail_id':
			...
			$q = "INSERT IGNORE INTO `#__miwoshop_product` (`product_id`, `model`, `sku`, `location`, `quantity`, `stock_status_id`, `image`, `shipping`, `price`, `tax_class_id`, `status`,".
				"  `date_added`, `date_modified`, `date_available`, `sort_order`)".
				" VALUES ('".$pro['product_id']."', '".$pro['product_sku']."', '".$pro['product_sku']."', '', '".$stock_quantity."', '7', '".$pro_image."', '1', '".$pro_price."', '', '".$pro_publish.
				"', '".$datec."', '".$datem."',  '".$datec."', '".$pro['menu_order']."')";
			$db->setQuery($q);
			$db->query();
