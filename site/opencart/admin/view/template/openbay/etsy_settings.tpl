  ...
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right">
        <button type="submit" form="form-etsy-settings" data-toggle="tooltip" title="<?php echo $button_save; ?>" class="button button-primary" onclick="validateForm(); return false;"><i class="fa fa-check-circle"></i></button>
      ...
      <div class="tab-content">
        <div class="tab-pane active" id="tab-general">
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10"> <a class="button button-primary" id="button-import"><i class="fa fa-refresh"></i></a> </div>
          </div>
          <div class="form-group">
            ...
            <div class="col-sm-10"> <a class="button button-primary" id="button-settings"><i class="fa fa-refresh"></i></a> </div>
          </div>
        </div>
      </div>
    </form>
...
<script type="text/javascript"><!--
  ...
  $('#button-import').bind('click', function() {
    $.ajax({
      ...
      beforeSend: function(){
        $('#button-import').removeClass('btn-success').removeClass('btn-danger').addClass('button-primary').empty().html('<i class="fa fa-cog fa-lg fa-spin"></i>').attr('disabled','disabled');
      },
      type: 'post',
      dataType: 'json',
      success: function(json) {
        if (json.header_code == 200) {
          $('#button-import').empty().removeClass('button-primary').addClass('btn-success').html('<?php echo $text_complete; ?>');
          alert('<?php echo $text_orders_imported; ?>');
        } else {
          $('#button-import').empty().removeClass('button-primary').addClass('btn-danger').html('<?php echo $text_failed; ?>').removeAttr('disabled');
  ...
  $('#button-settings').bind('click', function() {
    $.ajax({
      ...
      beforeSend: function(){
        $('#button-settings').removeClass('btn-success').removeClass('btn-danger').addClass('button-primary').empty().html('<i class="fa fa-cog fa-lg fa-spin"></i>').attr('disabled','disabled');
      },
      type: 'get',
      dataType: 'json',
      success: function(json) {
        if (json.header_code == 200) {
          $('#button-settings').empty().removeClass('button-primary').addClass('btn-success').html('<?php echo $text_complete; ?>');
        } else {
          $('#button-settings').empty().removeClass('button-primary').addClass('btn-danger').html('<?php echo $text_failed; ?>').removeAttr('disabled');
        }
      },
      error: function (xhr, ajaxOptions, thrownError) {
        $('#button-settings').empty().removeClass('button-primary').addClass('btn-danger').html('<?php echo $text_failed; ?>').removeAttr('disabled');
