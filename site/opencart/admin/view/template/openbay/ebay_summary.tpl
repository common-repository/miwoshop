        ...
        <div class="well">
          <div class="row">
            <div class="col-sm-12 text-right">
              <a class="button button-primary" id="load-usage"><i class="fa fa-cog fa-lg fa-spin"></i></a>
            </div>
          </div>
        </div>
      </div>
  ...
  function loadSummary(){
    $.ajax({
      ...
      error: function (xhr, ajaxOptions, thrownError) {
        $('#load-usage').empty().removeClass('button-primary').addClass('btn-danger').html('<?php echo $text_failed; ?>').removeAttr('disabled');
