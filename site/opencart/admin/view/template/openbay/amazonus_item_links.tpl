        ...
        <div class="well">
          <div class="row">
            <div class="col-sm-12 text-right">
              <a class="button button-primary" id="button-load"><?php echo $button_load; ?></a>
            </div>
          </div>
        </div>
        <table class="table table-bordered table-hover">
          ...
          <tr>
            ...
            <td class="text-center">
              <a class="button button-primary" id="add-new-button" onclick="addNewLinkAutocomplete()" data-toggle="tooltip" data-original-title="<?php echo $button_add; ?>"><i class="fa fa-plus-circle"></i></a>
            </td>
          </tr>
          </tbody>
        </table>
  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      for (i in json) {
        ...
        rows += '<td class="text-center"><a data-toggle="tooltip" data-original-title="<?php echo $button_remove; ?>" class="button btn-danger" onclick="removeLink(this, \'' + json[i]['amazon_sku'] + '\');"><i class="fa fa-times-circle"></i></a></td>';
        rows += '</tr>';
      }
  ...
  $.ajax({
    ...
    success: function (json) {
      ...
      for (i in json) {
        ...
        rows += '<a class="button button-primary" onclick="addNewSkuField(' + json[i]['product_id'] + ', \'' + json[i]['var'] + '\')" data-toggle="tooltip" data-original-title="<?php echo $button_add; ?>"><i class="fa fa-plus-circle"></i></a>';
        rows += '</div>';
        rows += '</div>';
        rows += '</div>';
        rows += '</td>';
        rows += '<td class="text-center"><a class="button button-primary" onclick="addNewLink(this, \'' + json[i]['product_id'] + '\', \'' + json[i]['var'] + '\')" data-toggle="tooltip" data-original-title="<?php echo $button_add; ?>"><i class="fa fa-plus-circle"></i></a></td>';
        rows += '</tr>';
      }
...
function addNewSkuField(product_id, variation) {
  ...
  html += '<a class="button btn-danger remove_sku_icon_' + product_id + '_' + variation + '" onclick="removeSkuField(this, \'' + product_id + '\', \'' + variation + '\')"><i class="fa fa-minus-circle"></i></a>';
  html += '</div>';
  html += '</div>';
  html += '</div>';
