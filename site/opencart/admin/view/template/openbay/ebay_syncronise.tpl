  ...
  <div class="page-header">
    <div class="container-fluid">
      <div class="pull-right">
        <a href="<?php echo $return; ?>" data-toggle="tooltip" title="<?php echo $button_back; ?>" class="button btn-default"><i class="fa fa-reply"></i></a></div>
        ...
        <?php if ($validation == true) { ?>
          ...
          <form id="form-ebay-sync" class="form-horizontal">
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <a class="button button-primary" id="update-categories"><?php echo $button_update; ?></a>
              </div>
            </div>
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <a class="button button-primary" id="update-store"><?php echo $button_update; ?></a>
              </div>
            </div>
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <a class="button button-primary" id="update-settings"><?php echo $button_update; ?></a>
              </div>
            </div>
          </form>
        <?php }else{ ?>
...
<script type="text/javascript"><!--
  $('#update-categories').bind('click', function() {
    $.ajax({
      ...
      success: function(json) {
        $('#update-categories').empty().removeClass('button-primary').addClass('btn-success').html('<?php echo $text_complete; ?>');
        alert(json.msg);
      },
      error: function (xhr, ajaxOptions, thrownError) {
        $('#update-categories').empty().removeClass('button-primary').addClass('btn-danger').html('<?php echo $error_failed; ?>').removeAttr('disabled');
  ...
  $('#update-settings').bind('click', function() {
    $.ajax({
      ...
      dataType: 'json',
      success: function(json) {
        $('#update-settings').empty().removeClass('button-primary').addClass('btn-success').html('<?php echo $text_complete; ?>');
      ...
      error: function (xhr, ajaxOptions, thrownError) {
        $('#update-settings').empty().removeClass('button-primary').addClass('btn-danger').html('<?php echo $error_failed; ?>').removeAttr('disabled');
        if (xhr.status != 0) { alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText); }
      }
    });
  });
  ...
  $('#update-store').bind('click', function() {
    $.ajax({
      ...
      dataType: 'json',
      success: function(json) {
        $('#update-store').empty().removeClass('button-primary').addClass('btn-success').html('<?php echo $text_complete; ?>');
      ...
      error: function (xhr, ajaxOptions, thrownError) {
        $('#update-store').empty().removeClass('button-primary').addClass('btn-danger').html('<?php echo $error_failed; ?>').removeAttr('disabled');
        if (xhr.status != 0) { alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText); }
      }
    });
  });
