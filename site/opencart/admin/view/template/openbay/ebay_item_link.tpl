  ...
  <div class="container-fluid">
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        <div class="buttons"> <a class="button button-primary" id="check-unlinked-items"><?php echo $button_check_unlinked; ?></a>
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        <table class="table table-bordered table-hover">
          ...
          <tr>
            <td class="text-left" colspan="8" id="checking-linked-items"><a class="button button-primary" id="load-usage"><i class="fa fa-cog fa-lg fa-spin"></i> <?php echo $text_text_loading_items; ?></a></td>
          </tr>
...
<script type="text/javascript"><!--
                ...
                if (val.variants == 0) {
                  ...
                  if (val.status == 1) {
                    if (val.qty == store_qty || val.qty == reserve_qty) {
                      ...
                      $('#text-buttons-'+key).html('<a href="<?php echo $edit_url; ?>'+product_id+'" class="button button-primary"><span><?php echo $button_edit; ?></span></a>');
                    } else {
                      $('#text-status-'+key).text('<?php echo $text_stock_error; ?>');
                      $('#row-'+key+' > td').css('background-color', '#FFD4D4');
                      $('#text-buttons-'+key).html('<a onclick="updateLink('+key+','+val.qty+','+product_id+', '+store_qty+', '+reserve_qty+');" class="button button-primary"><span><?php echo $button_resync; ?></a>');
                    }
                  } else {
                    $('#text-status-'+key).text('<?php echo $text_listing_ended; ?>');
                    $('#row-'+key+' > td').css('background-color', '#FFD4D4');
                    $('#text-buttons-'+key).html('<a onclick="removeLink('+product_id+', '+key+');" class="button btn-danger"><i class="fa fa-minus-circle fa-lg"></i> <?php echo $button_remove_link; ?></a>');
                  }
                } else {
          ...
          $.ajax({
              ...
              success: function(json) {
                  if (json.error == false) {
                      ...
                      $('#text-buttons-'+item_id).empty().html('<a href="<?php echo $edit_url; ?>'+product_id+'" class="button button-primary"><?php echo $button_edit; ?></a>');
  ...
  $('#check-unlinked-items').bind('click', function() {
    ...
    $.ajax({
      ...
      success: function(json) {
        if (json.data.items === null) {
          ...
          $.each (json.data.items, function(key, val) {
            ...
            html_inj += '<td class="text-center"><a class="button button-primary" style="display:none;" onclick="saveListingLink('+key+'); return false;" id="l-'+key+'-save-button"><span><?php echo $button_save; ?></span></a></td>';
            html_inj += '</tr>';
      ...
      error: function (xhr, ajaxOptions, thrownError) {
        $('#check-unlinked-items').empty().removeClass('button-primary').addClass('btn-danger').html('<?php echo $text_failed; ?>').removeAttr('disabled');
        if (xhr.status != 0) { alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText); }
      }
    });
  });
