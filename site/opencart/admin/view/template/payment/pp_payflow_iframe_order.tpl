...
<table class="table table-striped table-bordered">
  ...
  <tr>
    ...
    <td id="complete-entry"><?php if ($complete) { ?>
      ...
      <a class="button button-primary" id="button-capture" onclick="capture()"><?php echo $button_capture ?></a>
      <?php } ?></td>
  </tr>
  <tr>
    ...
    <td id="reauthorise-entry"><?php if ($complete) { ?>
      -
      <?php } else { ?>
      <a class="button button-primary" id="button-void" onclick="doVoid()"><?php echo $button_void ?></a>
      <?php } ?></td>
  </tr>
  <tr>
...
function doVoid() {
    if (confirm('<?php echo $text_confirm_void; ?>')) {
        $.ajax({
            ...
            beforeSend: function() {
                $('#button-void').after('<span class="button button-primary loading"><i class="fa fa-circle-o-notch fa-spin fa-lg"></i></span>');
                $('#button-void').hide();
            },
...
function capture() {
    ...
    $.ajax({
        ...
        beforeSend: function() {
            $('#button-capture').after('<span class="button button-primary loading"><i class="fa fa-circle-o-notch fa-spin fa-lg"></i></span>');
            $('#button-capture').hide();
        },
