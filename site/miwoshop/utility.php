...
class MiwoShopUtility {
    ...
    function getMiwoshopIcon($link, $image, $text) {
    	...
    	<div <?php echo $div_class; ?> style="float:<?php echo ($lang->isRTL()) ? 'right' : 'left'; ?>;">
    		<div class="icon">
    			<a href="<?php echo $link; ?>">
    				<img src="<?php echo MURL_MIWOSHOP; ?>/admin/assets/images/<?php echo $image; ?>" alt="<?php echo $text; ?>" />
    ...
    public function copyLang($language_id){
        ...
        $def_lang_id = MiwoShop::get('opencart')->get('config')->get('config_language_id');
        ...
        foreach ($query as $attribute) {
            $db->run("INSERT IGNORE INTO #__miwoshop_attribute_description SET attribute_id = '" . (int)$attribute['attribute_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($attribute['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $attribute_group) {
            $db->run("INSERT IGNORE INTO #__miwoshop_attribute_group_description SET attribute_group_id = '" . (int)$attribute_group['attribute_group_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($attribute_group['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $banner_image) {
            $db->run("INSERT IGNORE INTO #__miwoshop_banner_image_description SET banner_image_id = '" . (int)$banner_image['banner_image_id'] . "', banner_id = '" . (int)$banner_image['banner_id'] . "', language_id = '" . (int)$language_id . "', title = '" . $db->run($banner_image['title'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $customer_group) {
            $db->run("INSERT IGNORE INTO #__miwoshop_customer_group_description SET customer_group_id = '" . (int)$customer_group['customer_group_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($customer_group['name'], 'escape') . "', description = '" . $db->run($customer_group['description'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $download) {
            $db->run("INSERT IGNORE INTO #__miwoshop_download_description SET download_id = '" . (int)$download['download_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($download['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $filter) {
            $db->run("INSERT IGNORE INTO #__miwoshop_filter_description SET filter_id = '" . (int)$filter['filter_id'] . "', language_id = '" . (int)$language_id . "', filter_group_id = '" . (int)$filter['filter_group_id'] . "', name = '" . $db->run($filter['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $filter_group) {
            $db->run("INSERT IGNORE INTO #__miwoshop_filter_group_description SET filter_group_id = '" . (int)$filter_group['filter_group_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($filter_group['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $information) {
            $db->run("INSERT IGNORE INTO #__miwoshop_information_description SET information_id = '" . (int)$information['information_id'] . "', language_id = '" . (int)$language_id . "', title = '" . $db->run($information['title'], 'escape') . "', description = '" . $db->run($information['description'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $length) {
            $db->run("INSERT IGNORE INTO #__miwoshop_length_class_description SET length_class_id = '" . (int)$length['length_class_id'] . "', language_id = '" . (int)$language_id . "', title = '" . $db->run($length['title'], 'escape') . "', unit = '" . $db->run($length['unit'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $option) {
            $db->run("INSERT IGNORE INTO #__miwoshop_option_description SET option_id = '" . (int)$option['option_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($option['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $option_value) {
            $db->run("INSERT IGNORE INTO #__miwoshop_option_value_description SET option_value_id = '" . (int)$option_value['option_value_id'] . "', language_id = '" . (int)$language_id . "', option_id = '" . (int)$option_value['option_id'] . "', name = '" . $db->run($option_value['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $order_status) {
            $db->run("INSERT IGNORE INTO #__miwoshop_order_status SET order_status_id = '" . (int)$order_status['order_status_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($order_status['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $product_attribute) {
            $db->run("INSERT IGNORE INTO #__miwoshop_product_attribute SET product_id = '" . (int)$product_attribute['product_id'] . "', attribute_id = '" . (int)$product_attribute['attribute_id'] . "', language_id = '" . (int)$language_id . "', text = '" . $db->run($product_attribute['text'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $return_action) {
            $db->run("INSERT IGNORE INTO #__miwoshop_return_action SET return_action_id = '" . (int)$return_action['return_action_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($return_action['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $return_reason) {
            $db->run("INSERT IGNORE INTO #__miwoshop_return_reason SET return_reason_id = '" . (int)$return_reason['return_reason_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($return_reason['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $return_status) {
            $db->run("INSERT IGNORE INTO #__miwoshop_return_status SET return_status_id = '" . (int)$return_status['return_status_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($return_status['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $stock_status) {
            $db->run("INSERT IGNORE INTO #__miwoshop_stock_status SET stock_status_id = '" . (int)$stock_status['stock_status_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($stock_status['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $voucher_theme) {
            $db->run("INSERT IGNORE INTO #__miwoshop_voucher_theme_description SET voucher_theme_id = '" . (int)$voucher_theme['voucher_theme_id'] . "', language_id = '" . (int)$language_id . "', name = '" . $db->run($voucher_theme['name'], 'escape') . "'", 'query');
        }
        ...
        foreach ($query as $weight_class) {
            $db->run("INSERT IGNORE INTO #__miwoshop_weight_class_description SET weight_class_id = '" . (int)$weight_class['weight_class_id'] . "', language_id = '" . (int)$language_id . "', title = '" . $db->run($weight_class['title'], 'escape') . "', unit = '" . $db->run($weight_class['unit'], 'escape') . "'", 'query');
        }
    }

    public function synchronizeAliasManually(){
        $db         = MiwoShop::get('db');
        $def_lang   = MiwoShop::get('opencart')->get('config')->get('config_language_id');
        ...
        if (!empty($aliases)) {
            foreach ($languages as $language) {
                ...
                $is_sync = $db->run("SELECT COUNT(url_alias_id) FROM `#__miwoshop_url_alias` WHERE language_id = {$language['language_id']}", 'loadResult');

                if($is_sync > 0){
                    continue;
                }

                $db->run("INSERT INTO #__miwoshop_url_alias (query, keyword, language_id)
							SELECT query, keyword, ". $language['language_id'] ." as language_id
							FROM #__miwoshop_url_alias WHERE language_id = {$def_lang} AND query NOT LIKE 'manufacturer_id%'", 'query');
